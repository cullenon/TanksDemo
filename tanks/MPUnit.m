...
// Add sprite type and position to the game's map data
- (void)addSpriteToMapData:(NSString *)type x:(int)x y:(int)y {
    
    NSMutableArray *mapUnits = [theGame.mapData objectForKey:[NSString stringWithFormat:@"army%d",owner]];
    NSMutableDictionary * unitData = [NSMutableDictionary dictionary];
    [unitData setObject:type forKey:@"type"];
    [unitData setObject:[NSNumber numberWithInt:x] forKey:@"posX"];
    [unitData setObject:[NSNumber numberWithInt:y] forKey:@"posX"];
    NSLog(@"x:%d y:%d",x,y);
    [mapUnits addObject:unitData];
...
-(void)createSprite:(NSMutableDictionary *)tileDict {
    ...
    [self addSpriteToMapData:[tileDict valueForKey:@"Type"] x:x y:y];
...
-(void)doMarkedMovement:(TileData *)targetTileData {
    ...
    _tileDataBeforeMovement = [theGame getTileData:startTile];
    [self insertOrderedInOpenSteps:_tileDataBeforeMovement];
    do {
...
-(void)doStay {
    ...
    if ([self isKindOfClass:[MPUnit_Soldier class]]) {
    ...
    int x = mySprite.position.x;
    int y = mySprite.position.y;
    NSMutableArray *mapUnits = [theGame.mapData objectForKey:@"move_queue"];
    NSMutableDictionary * unitData = [NSMutableDictionary dictionary];
    //[unitData setObject:type forKey:@"type"];
    [unitData setObject:[NSNumber numberWithInt:x] forKey:@"posX"];
    [unitData setObject:[NSNumber numberWithInt:y] forKey:@"posX"];
...
-(void)doCancel {
    ...
    mySprite.position = [theGame positionForTileCoord:_tileDataBeforeMovement.position];
    [theGame unselectUnit];
}
...
-(void)dealDamage:(NSMutableDictionary *)damageData {
    ...
    MPUnit *attacker = [damageData objectForKey:@"attacker"];
    // 2 - Calculate damage
