...
- (void)loadUnits:(int)player {
    ...
    for (NSMutableDictionary * unitDict in [unitsObjectGroup objects]) {
        ...
        Unit * unit = [Unit_Soldier nodeWithTheGame:self tileDict:d owner:player];
...
// Restart game
-(void)restartGame {
    [[CCDirector sharedDirector] replaceScene:[CCTransitionJumpZoom transitionWithDuration:1 scene:[TurnLayer scene]]];
}
